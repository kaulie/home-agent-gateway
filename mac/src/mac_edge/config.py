...
def load_config() -> Config:
    root = _project_root()
    # 部署系统（agent-control-plane-deployment）只保留 <runtime>/backend/ 下的运行期文件；
    # 生产把 .env 放 <repo>/backend/.env。先加载它（_load_dotenv 里「已存在的环境变量优先」，
    # 所以 backend/.env 优先于 mac/.env），本地开发仍按 mac/.env 走。
    _load_dotenv(_repo_root() / "backend")
    _load_dotenv(root)
    data_dir = Path(
